...
import org.aincraft.container.IPotionModifier.ModifierType;
...
final class ModifierNodeRegistryFactory implements IFactory<IRegistry<ModifierNode>> {
  ...
  ModifierNodeRegistryFactory(Plugin plugin,
      IYamlConfiguration configuration, ItemParser parser,
  ...
  private static final class ModifierNodeFactory {
    ...
    ModifierNodeFactory(Plugin plugin, ItemParser parser, IDurationStageRegistry durationRegistry) {
      this.modifierFactory = new ModifierFactory(durationRegistry);
      this.plugin = plugin;
      this.parser = parser;
    }

    public ModifierNode create(@NotNull ConfigurationSection section)
      ...
      ItemStack stack = parser.parse(itemKeyString);
      ...
      return new ModifierNode(new NamespacedKey(plugin, keyString),
          new CauldronIngredient(stack, amount), modifiers);
    }
  }
  ...
  @Override
  public IRegistry<ModifierNode> create() {
    ...
    SimpleRegistry<ModifierNode> modifierNodes = new SimpleRegistry<>();
    for (String modifierNodeKey : modifierSection.getKeys(false)) {
      ...
      try {
        ModifierNode modifierNode = modifierNodeFactory.create(modifierNodeSection);
        modifierNodes.register(modifierNode.getKey(), modifierNode);
